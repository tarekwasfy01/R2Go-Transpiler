...
func do_unz(call, op, args, env Value) Value {
	...
	Assign(LocalRef(&con), RT.AssignIndex(RT.Symbol("Connections"), ncon, RT.Call("R_newunz", file, func() Value {
	...
	RT.Call("SET_STRING_ELT", class, RT.Const("int", "0"), RT.Call("mkChar", RT.Const("string", "\"unz\"")))
