...
func do_lapack(call, op, args, env Value) Value {
	...
	if RT.Truth(RT.Binary(">", RT.Symbol("initialized"), RT.Const("int", "0"))) {
		return RT.CallIndirect(RT.Deref(RT.Field(RT.Symbol("ptr"), "do_lapack")), call, op, args, env)
	} else {
		RT.Call("error", RT.Call("_", RT.Const("string", "\"LAPACK routines cannot be loaded\"")))
